...
const App = () => {
  ...
  return (
    <div>
      <br />
      <div className="bg-[#98FB98]/[0.5] border-2 shadow-lg max-w-3xl rounded-lg p-3 m-auto">
        <Title title="Add To do List" />
        <Input placeholder="Masukan Inputan Task" value={newTaskValue} onChange={(event) => setNewTaskValue(event.target.value)} />
        <Button text={isBool ? "update" : "Tambah Task"} onClick={actionAddTask} />
      </div>
      <div>
        <ul>
